...
export async function GET(
  ...
  try {
    const pageRes = await db.query(
      `SELECT id, name, description, profile_image, slug, expires_at, views, clicks, is_official, show_footer, sponsor_name, sponsor_phone
       FROM linktree_pages WHERE slug = $1 AND is_official = false LIMIT 1`,
      [slug.trim()]
    );
    ...
    const status = computePageStatus(page.expires_at.toISOString().slice(0, 10));
